...
describe("Twitter component", () => {
  ...
  it("Displays correct image", async () => {
    ...
    act(() => {
      render(<TwitterCard avatar={response.data.avatar} name={response.data.name}>
        {response.data.comment}
      </TwitterCard>);
    });
    
    await waitFor(() => {
      var imgTag = screen.getAllByText((content, element) => element.tagName.toLowerCase() === "img")[0];
